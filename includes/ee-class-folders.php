	...
	public function eeSFLF_FunctionBar($eeSFLF_ListFolder, $eeSFLF_ShortcodeFolder) {
		...
		if($eeShowCreate) {
			...
			$eeForm .= '
				...
				<input class="button" type="submit" value="' . __('Create Folder', 'ee-simple-file-list-folders') . '" />
		...
		if(($eeAdmin OR $eeSFL_Config['ShowBreadCrumb'] == 'YES') AND !@$_POST['eeSFL_Upload']) {
			...
			if( @$_GET['eeFolder'] ) {  // OR strpos($eeSFLF_ListFolder, '/')
				...
				if(!$eeSFLF_ShortcodeFolder) {
					$eeTrail .= '<a href="' . $eeSFLF_BaseURL . '">' . __('Home', 'ee-simple-file-list-folders') . '</a> / '; // Never show the Home folder if a shortcode folder was used
				}
					...
					foreach( $eeSFLF_FolderArray as $key => $folder) {
						...
						if($eeSFLF_ShortcodeFolder) { // Shortcode defined folder. Omit folders up to here
							...
							if($i != $count) {
								...
								if($i == 1) { $folder = __('Home', 'ee-simple-file-list-folders'); } // We are within a sub-folder, change base folder name to "Home"
								...
								$eeTrail .= __('Home', 'ee-simple-file-list-folders') . ' / ';
							}
