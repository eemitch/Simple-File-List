...
$eeSFL_FREE_Log['Add Files'][] = 'Loaded: ee-uploader';

// Extension Check
if($eeSFLF) {
	if(@$_REQUEST['eeFolder'] AND $eeSFL_FREE_ListRun == 1) { // Adjust the path based on REQUEST arg
...
$eeSFL_FREE_Log['Add Files'][] = 'Uploading to...';
$eeSFL_FREE_Log['Add Files'][] = $eeSFL_FREE_Config['FileListDir'] . $eeSFLF_ListFolder;
...
if(@$eeSFL_FREE_Config['FileListDir']) {
	...
	<!-- Simple File List Uploader -->
		...
		<script>
			...
			var eeSFL_FileUploadDir = "' . urlencode($eeSFL_FREE_Config['FileListDir'] . $eeSFLF_ListFolder) . '";
