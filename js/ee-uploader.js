...
function eeSFL_UploadProcessor(eeSFL_FileObjects) {
	...
	if(eeSFL_FileCount) {
		...
		if( jQuery('#eeSFL_Name').length > 0 ) { // Get Uploader Info is ON
			eeSFL_Name = jQuery('#eeSFL_Name').val();
			eeSFL_Email = jQuery('#eeSFL_Email').val();
...
function eeSFL_UploadFile(eeSFL_File) { // Pass in file object
	...
	var eeSFL_UploadNonce = jQuery('#eeSFL_UploadNonce').val(); // Get the Nonce
    ...
    eeFormData.append("eeSecurity", eeSFL_UploadNonce);
